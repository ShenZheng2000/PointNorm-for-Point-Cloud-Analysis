...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        # NOTE: add concat if use_xyz
        if self.use_xyz:
            grouped_points = torch.cat([grouped_points, grouped_xyz],dim=-1)
            new_points = torch.cat([new_points, new_xyz],dim=-1)
...
class PreExtraction(nn.Module):
    def __init__(self, channels, out_channels,  blocks=1, groups=1, res_expansion=1, bias=True,
        ...
        in_channels = 6+2*channels if use_xyz else 2*channels # NOTE: use '6' here to allow 3 add. channel for new_points 
...
def PointNorm(num_classes=50, 
                ...
                global_mean=False,
                global_std=False,
                use_xyz=True, # NOTE: use_xyz default is True
                **kwargs):
    return Model(num_classes=num_classes, points=2048, embed_dim=embed_dim, groups=1, res_expansion=res_expansion,
                 activation="relu", bias=True, use_xyz=use_xyz, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def PointNormTiny(num_classes=50, 
                    embed_dim = 32, 
                    res_expansion = 0.25, 
                    use_inv_res = False, 
                    point_norm=False, 
